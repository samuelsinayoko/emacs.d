...
(defun sam-ipython-terminal (&optional other-ipython-p)
  ...
  (let (cmd)
    ...
    (set-buffer (make-term sam-ipython-process-name cmd nil "--simple-prompt"))
...
statement. "
  ...
  (let (start end)
      (beginning-of-line)
      (setq start (point))
      (unless (py-goto-statement-below)
	;; if there is not statement below try going behond the block
...
(define-minor-mode sam-ipython-mode
  ...
  :keymap (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-M-<return>") 'sam-ipython-run-statement)
