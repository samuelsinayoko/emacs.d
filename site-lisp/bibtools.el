...
;;;     FAX             = "+1 801 581 4148",
;;;     checksum        = "41596 3939 15215 132041",
...
(defconst bibtools-version "1.86 [12-May-2012]" "Version of bibtools library")
...
variable."
  (let ((abbrev nil) (begin) (s) (word) (wordlist))
    (re-search-backward "^[ \t]*@")
    (if (re-search-forward "[^a-zA-Z]title[ \t]*=[ \t]*\"" nil t)
	(progn
	  (setq begin (point))
	  (if (re-search-forward "\",[ \t]*$" nil t)
	      (progn
...
(defun fix-mathscinet-accents ()
  ...
  (goto-char (point-min))
  (replace-string "é" "{\\'e}" nil)
  (goto-char (point-min))
  (replace-string "è" "{\\`e}" nil)
  (goto-char (point-min))
  (replace-string "à" "{\\`a}" nil)
  (goto-char (point-min))
  (replace-string "ä" "{\\\"a}" nil)
  (goto-char (point-min))
  (replace-string "Ö" "{\\\"O}" nil)
  (goto-char (point-min))
  (replace-string "É" "{\\'E}" nil)
  (goto-char (point-min))
  (replace-string "ü" "{\\\"u}" nil)
  (goto-char (point-min))
  (replace-string "ý" "{\\'y}" nil)
  (goto-char (point-min))
  (replace-string "ø" "{\\o}" nil)
  (goto-char (point-min))
  (replace-string "ó" "{\\'o}" nil)
  (goto-char (point-min))
  (replace-string "ö" "{\\\"o}" nil)
  (goto-char (point-min))
  (replace-string "è" "{\\`e}" nil)
  (goto-char (point-min))
  (replace-string "ç" "{\\c{c}}" nil)
  (goto-char (point-min))
  (replace-string "á" "{\\'a}" nil)
  (goto-char (point-min))
  (replace-string "È" "{\\`E}" nil)
  (goto-char (point-min))
  (replace-string "ú" "{\\'u}" nil)
  )
...
(defun title-replace (old new)
  "Run a replace-string of OLD by NEW, but in the title fields only, in
a BibTeX file."
  (interactive)
  (let ((end))
    (while (re-search-forward "^[ \t]*title[ \t]*=[ \t]*\"\\|^[ \t]*booktitle[ \t]*=[ \t]*\"" nil t)
